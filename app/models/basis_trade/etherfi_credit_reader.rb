...
# Reads the current Ether.fi Credit debt for a Cash Safe from the official Cash
# LendGateway. This reader is intentionally separate from CashLoanReader, whose
# direct-borrow event ledger remains the Basis-monitoring source.
class BasisTrade::EtherfiCreditReader
  ...
  # AccountData has five uint256 words; debtUsd is word two (index one).
  def borrowing_usd(vault_address:)
    ...
    BigDecimal(decode_debt_usd(raw).to_s) / (10 ** USD_DECIMALS)
  end
